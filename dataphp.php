...
if($_SERVER["REQUEST_METHOD"]==="POST"){
    header("Location:/dataphp.php");
}
try {
    $pdo = new PDO("mysql:host=localhost;dbname=dataphp;charset=utf8","root","password",array(PDO::ATTR_EMULATE_PREPARES => false));
  $item_name = $_POST["textbox"];
  if ($item_name && isset($_POST["btn"])) {
    unset($_POST["btn"]);
  $item_point = $_POST["number"];
  $stmt = $pdo -> prepare("INSERT INTO tablephp(name, point) VALUES (:name, :point)");
  $stmt->bindParam(':name', $item_name, PDO::PARAM_STR);
  $stmt->bindValue(':point', $item_point, PDO::PARAM_INT);
  $stmt->execute();
    }
    $select_sql = "SELECT * FROM tablephp";
  ...
  <body>
    <form action="dataphp.php" method="post">
    <div class="header">
        <h1>評価.com</h1>
      </div>
    </div>
